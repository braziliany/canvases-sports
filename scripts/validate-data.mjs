...
for (const config of RESULT_SOURCES) {
  ...
  const adapter = RESULT_ADAPTERS.get(snapshot.adapter);
...
if (rankingReference.some((reference) => reference.rows.length !== published.standings.length ||
  reference.requiredFixtureIds.some((id) => !fixtures.fixtures.some((fixture) => fixture.id === id)))) {
